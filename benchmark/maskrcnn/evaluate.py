...
"""Chấm một model trên một split, qua một đường chung cho mọi model.
    ...
    python benchmark/maskrcnn/evaluate.py --config benchmark/maskrcnn/configs/eval/maskrcnn_r50_d2.yaml --weights <best.pt>
    python benchmark/maskrcnn/evaluate.py --config benchmark/maskrcnn/configs/eval/maskrcnn_r50_d2.yaml --weights <best.pt> --data data/export/field/f1 --split test
    python benchmark/maskrcnn/evaluate.py --config benchmark/maskrcnn/configs/eval/maskrcnn_r50_d2.yaml --size t --limit 5
...
def main() -> int:
    ...
    if not a.config:
        raise SystemExit("Cần --config <benchmark/maskrcnn/configs/eval/...yaml>, hoặc --native --weights <best.pt>.")
