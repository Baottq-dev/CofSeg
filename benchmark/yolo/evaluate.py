...
#
#     python benchmark/yolo/evaluate.py --config benchmark/yolo/configs/eval/yolo11s.yaml
"""Chấm một model trên một split, qua một đường chung cho mọi model.

    python benchmark/yolo/evaluate.py --config benchmark/yolo/configs/eval/yolo11s.yaml --weights <best.pt>
    python benchmark/yolo/evaluate.py --config benchmark/yolo/configs/eval/yolo11s.yaml --weights <best.pt> --data data/export/field/f1 --split test
    python benchmark/yolo/evaluate.py --config benchmark/yolo/configs/eval/yolo11s.yaml --size t --limit 5
    python benchmark/yolo/evaluate.py --config benchmark/yolo/configs/eval/_coco.yaml --file preds/cascade.json
...
Đường riêng cho YOLO — `model.val()` của ultralytics, đúng thứ `yolo val` chạy:
    ...
    python benchmark/yolo/evaluate.py --native --weights <best.pt> --split test --imgsz 1024
...
def main() -> int:
    ...
    if not a.config:
        raise SystemExit("Cần --config <benchmark/yolo/configs/eval/...yaml>, hoặc --native --weights <best.pt>.")
