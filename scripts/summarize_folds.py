...
Đọc runs/eval/<...>/{config.yaml,metrics.json} của các lần chấm tên
...
def main() -> int:
    ...
    ap.add_argument("--eval", default="runs/eval")
    ...
    if not rows:
        print(f"Không thấy lần chấm nào tên <model>_<fold> trong {a.eval}")
        return 1
    if a.out:
        out = Path(a.out)
