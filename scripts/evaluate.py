...
Đường riêng cho YOLO — `model.val()` của ultralytics, đúng thứ `yolo val` chạy:
    ...
    python scripts/evaluate.py --native --weights <best.pt> --split test --imgsz 1024
...
def run_config(a, extra: list[str]) -> int:
    ...
    data = {"root": "data/export/dataset_v1", "split": "test", "min_area": 50.0,
            **(cfg.get("data") or {})}
    if a.split:
        data["split"] = a.split
...
def main() -> int:
    ...
    ap.add_argument("--data", default="data/export/dataset_v1/data.yaml", help="[native] data.yaml")
