...
# code/config, chạy khi gọi tên qua --only. Config của mỗi model nằm trong thư
# mục người phụ trách (members/<model>/configs/), xem members/README.md.
set -euo pipefail
...
if want yolo11s; then
  ...
  python scripts/train.py --config members/yolo11/configs/train/yolo11s.yaml \
  ...
  python scripts/evaluate.py --config members/yolo11/configs/eval/yolo11s.yaml \
...
run_cfg() {   # $1 = tên model trong preds, $2 = đường dẫn config train
  NAME="$(basename "$2" .yaml)-$FOLD"
  SM=(); [ $SMOKE = 1 ] && SM=(--set data.limit=16 --epochs 1)
  python scripts/train.py --config "$2" \
...
want maskrcnn    && run_cfg maskrcnn    members/maskrcnn/configs/train/maskrcnn_r50_d2.yaml
want solov2      && run_cfg solov2      members/solov2/configs/train/solov2_r50_mm.yaml
want mask2former && run_cfg mask2former members/mask2former/configs/train/mask2former_r50_d2.yaml
want cascade     && run_cfg cascade     members/maskrcnn/configs/train/cascade_r50_d2.yaml
