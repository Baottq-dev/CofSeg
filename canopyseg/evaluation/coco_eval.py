...
def predictions_to_coco(image_id: int, preds, category_id: int = 1) -> list[dict]:
    ...
    for p in preds:
        ...
        out.append({
            ...
            "segmentation": encode_mask(p.mask),
            "score": float(p.score),
        })
    return out
...
def evaluate(
    gt_json: str, detections: list[dict], img_ids, dilation_ratio: float = BOUNDARY_DILATION_RATIO
) -> dict:
    ...
    b_stats, b_text = _run(BoundaryCOCOeval(gt, dt, dilation_ratio), img_ids)
    return {
        "mask": mask_stats,
        "boundary": b_stats,
        "mask_text": mask_text,
        "boundary_text": b_text,
